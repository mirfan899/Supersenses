...
@Model.register("streusle_tagger_roberta")
class StreusleTaggerRoberta(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self.roberta_config = AutoConfig.from_pretrained(f"roberta-{roberta_type}")
        self.roberta_config.output_hidden_states = True
        self.roberta = AutoModel.from_pretrained(f"roberta-{roberta_type}", config=self.roberta_config)
        ...
        if encoder is not None:
            check_dimensions_match(self.roberta.config.hidden_size, encoder.get_input_dim(),
                                   "roberta embedding dim", "encoder input dim")
        if feedforward is not None:
    ...
    def forward(self,  # type: ignore
        ...
        roberta_output = self.roberta(input_ids=input_ids, attention_mask=input_mask)
        ...
        embedded_wordpieces = self.scalar_mix(all_layer_embedded_wordpieces, mask)

        # Shape: (batch_size, 1)
        range_vector = util.get_range_vector(
                batch_size, util.get_device_of(token_indices_to_wordpiece_indices)
        ).unsqueeze(1)
        embedded_text_input = embedded_wordpieces[range_vector, token_indices_to_wordpiece_indices]
